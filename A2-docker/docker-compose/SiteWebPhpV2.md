...
## ALLEZ À localhost:8000 ou IP:8000

## REMPLIR LE FORMULAIRE ET VÉRIFICATION DE L'AJOUT VIA LA VALIDATION (TEXTE VERT QUI CONFIRME L'AJOUT)
...
# Annexe# 3 - SELinux (Security-Enhanced Linux).

```shell
# Pour passer SELinux en mode permissif temporairement
setenforce 0

# Pour vérifier le statut actuel de SELinux
getenforce

# Pour configurer SELinux en mode permissif de manière permanente, éditez le fichier /etc/selinux/config
# Utilisez un éditeur de texte comme nano ou vim. Par exemple:
sudo nano /etc/selinux/config
# Ou
sudo vim /etc/selinux/config

# Dans le fichier, modifiez la ligne suivante pour mettre SELinux en mode permissif
SELINUX=permissive
# Sauvegardez le fichier et redémarrez votre système pour que les modifications prennent effet.


# Pour revenir au mode enforcing (strict)
# Temporairement:
setenforce 1
# De manière permanente, modifiez à nouveau le fichier /etc/selinux/config:
sudo nano /etc/selinux/config
# Changez la ligne SELINUX pour qu'elle soit
SELINUX=enforcing
# Sauvegardez et redémarrez le système.
https://www.tecmint.com/disable-selinux-in-centos-rhel-fedora/
https://www.thegeekstuff.com/2009/06/how-to-disable-selinux-redhat-fedora-debian-unix/

```

# Annexe# 4 -  Utilisateur www-data:www-data de apache 
```shell
more /etc/passwd | grep www-data (utilisateur apache)
